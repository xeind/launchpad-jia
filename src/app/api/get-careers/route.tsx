...
export async function GET(req: Request) {
  ...
  try {
    ...
    let defaultSort: any = { status: 1, updatedAt: -1, _id: -1 };
    ...
    if (status && status !== "All Statuses") {
      if (status === "Published") {
        ...
        filter.isDraft = { $ne: true }; // Exclude drafts
      } else if (status === "Unpublished") {
        filter.status = "inactive";
      } else if (status === "Draft") {
        // Match either isDraft: true OR status: "draft"
        filter.$or = [{ isDraft: true }, { status: "draft" }];
      }
    }

    console.log("📊 GET CAREERS REQUEST:");
    ...
    console.log("📊 GET CAREERS RESULTS:");
    console.log("  - Total Careers (matching filter):", total);
    console.log("  - Careers Returned (current page):", careers.length);
    console.log("  - Total Active Careers:", totalActiveCareers);
